...
const Navbar = ({ cartCount, onCartPopupClose }) => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-[999] bg-white shadow-md py-2.5 px-12 flex justify-between items-center transition-top duration-300 ${
          visible ? "top-0" : "-top-[100px]"
        }`}
      >
        <div className="flex items-center">
          <span
            className="md:hidden text-3xl cursor-pointer text-gray-800 mr-4"
        ...
        <ul className="hidden md:flex items-center space-x-6">
          {navLinks.map(({ name, path, badge }) => (
            <li key={name}>
              ...
              >
                {name}
                {badge && (
                  <span className="bg-[#ff69b4] text-white text-xs px-1.5 py-0.5 rounded ml-1 align-super">
                    {badge}
                  </span>
                )}
              </Link>
          ...
          <li className="relative group">
            <span className="font-bold text-gray-600 cursor-pointer group-hover:text-black">
              Pages
            </span>
          ...
          <li>
            <a href="#" className="font-bold text-gray-600 hover:text-black">
              Buy Now{" "}
              <span className="bg-green-500 text-white text-xs px-1.5 py-0.5 rounded ml-1">
                Sale
              </span>
            </a>
          </li>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative cursor-pointer">
            ...
            <span className="absolute -top-2 -right-2 bg-[#ff69b4] text-white text-xs rounded-full px-1.5 py-0.5">
              {cartCount}
            </span>
          </div>
        </div>
      ...
      {mobileMenuOpen && (
        <div
          className="fixed top-0 left-0 w-full h-full bg-black/40 z-[2000]"
          onClick={handleMobileMenuClose}
        >
          <div
            className="bg-white w-[80vw] max-w-[320px] h-full shadow-lg p-6 relative"
              ...
              {navLinks.map(({ name, path, badge }) => (
                <li key={name}>
                  ...
                  >
                    {name}
                    {badge && (
                      <span className="bg-[#ff69b4] text-white text-xs px-1.5 py-0.5 rounded ml-1">
                        {badge}
                      </span>
                    )}
                  </Link>
